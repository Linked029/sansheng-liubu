# 三省六部 架构共识（2026-08-04 grilling 输出）
...
1. 硬规则层 — 用户确认的闸门，可解释可编辑（总是准奏/总是驳回/排除清单）
2. AI 多维打分 — 四维度权重从审批历史中在线学习，只排序不截断
3. 嵌入探索轨 — 近似内容推荐 + 低相似度破茧推荐，配额制保底

## 隐私立场

- 敏感内容不出本机
- embedding 和搜索优先走本地（SearXNG / sqlite-vec）
- 云端 API 作为后续升级项记入文档

## 数据范围

- 文章优先
- 架构为代码片段和结构化数据源留入口（兵部效率脚本等）
...
# 三省六部 P2 生产计划：探索卷宗 + RAG 问答

2026-08-04：架构共识落地。P2 拆为两阶段——先探索卷宗（双轨采集第二条轨），后 RAG 问答。

## P2-1~7：探索卷宗

- 新表：search_directions（方向输入）、search_terms（AI 拆解搜索词）、exploration_items（搜索结果卷宗）
- AI 方向拆词：用户输入方向描述 → AI 拆为 3~5 个搜索词
- 搜索引擎：本地优先（DuckDuckGo HTML 抓取），云端 API 作为升级项
- 入库管线：每词取前 N 条，去重+摘取正文 → 写入 exploration_items
- API：方向 CRUD、搜索词触发、卷宗列表、手动归档（归档后自动建 SM-2 复习、源连续归档 2 次建议加固定源）
- Web：探索卷宗页面（方向输入卡片 + 卷宗浏览 + 批量归档）

## P3：RAG 问答 + 打磨

- 原 V3.0 方案 P2（RAG 问答）+ P3（打磨）合并
- FTS5 全文索引 / sqlite-vec 向量检索、问答页、Ollama 离线、桌面通知、采集日志 UI、PWA 离线